...
const { clearStreetFootprint, streetLot, DOWNTOWN_LOBBIES } = await bundled('src/scene/night/building-layout.ts');
const { createKitbash } = await bundled('src/scene/night/towers.ts');
for (const tier of ['low', 'med', 'high']) {
  const kit = createKitbash({ tier, keepOut: [], clear: clearStreetFootprint });
  let checked = 0;
